...
<body class="container" style="background-color:#f4d442">
    <?php
  ...
  try {
            ...
            $con = connectDB();
            /*Customer who made most booking in each year*/
            $query = "CREATE OR REPLACE VIEW CustomerBikeCount AS
                            SELECT cal.year,c.customerID, c.Name, count(*) as count
            ...
            $ps->execute();
            $querycount = "SELECT year,customerID, Name, count
                            FROM CustomerBikeCount where (year,count) in (SELECT year,max(count) from CustomerBikeCount group by year);";
            $ps1 = $con->prepare($querycount);
            $ps1->setFetchMode(PDO::FETCH_ASSOC); 
            $ps1->execute();                       
            $data = $ps1->fetchAll();
            if (count($data) ==0) {
